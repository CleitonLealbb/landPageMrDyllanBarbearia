...
export async function GET(_request: Request, context: Context) {
  try {
    ...
    const rows = await prisma.servicePackage.findMany({ where: { barbershopId: shop.id, status: "ACTIVE", items: { every: { service: { status: "ACTIVE" } } } }, orderBy: [{ displayOrder: "asc" }, { name: "asc" }], select: { id: true, name: true, description: true, priceCents: true, durationMinutes: true, displayOrder: true, items: { orderBy: { displayOrder: "asc" }, select: { service: { select: packageServiceSelect } } } } })
    return NextResponse.json(rows.filter((row) => row.items.length >= 2).map((row) => { const services = row.items.map(({ service }) => ({ id: service.id, name: service.name, priceCents: service.priceCents, durationMinutes: service.durationMinutes, category: service.category })); return { id: row.id, name: row.name, description: row.description, priceCents: row.priceCents, originalPriceCents: services.reduce((sum, service) => sum + service.priceCents, 0), durationMinutes: row.durationMinutes, displayOrder: row.displayOrder, services } }))
