...
import { adminPackageSelect, buildPackageCreateData, presentPackage, validatePackageBody } from "@/lib/services/packages"
...
export async function POST(request: Request) {
  try {
    ...
    const createData = buildPackageCreateData(barbershopId, {
      name: data.name!,
    ...
    const row = await prisma.$transaction((tx) => tx.servicePackage.create({ data: createData, select: adminPackageSelect }))
